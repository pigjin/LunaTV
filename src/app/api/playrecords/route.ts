...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (key) {
      ...
      await Promise.all(
        Object.keys(records).map(async (recordKey) => {
          const [recordSource, recordId] = recordKey.split('+');
          if (!recordSource || !recordId) return;
          await db.deletePlayRecord(username, recordSource, recordId);
        })
      );
    }
